...
export class AuthService {
  private apiUrl = 'http://localhost:8980/api/auth'; // URL de ton backend
  private apiUrls = 'http://localhost:8980/api/user'; // URL de ton backend
    ...
    return this.http.post<any>(`${this.apiUrl}/login`, credentials).pipe(
      tap(response => {
        if (response && response.token) {
          ...
          // Stockez les informations utilisateur directement
          if (response.user) {
